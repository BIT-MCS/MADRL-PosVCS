...
class EnvRunner(Runner):
    ...
    def collect(self, step):
        ...
        if self.envs.action_space[0].__class__.__name__ == 'MultiDiscrete':
            ...
            # actions  --> actions_env : shape:[10, 1] --> [5, 2, 5] 和动作空间一致
    ...
    def insert(self, data):
        ...
        # 在这里加进去mask的内容, 原来没有
        if self.envs.envs[0].env.config['use_masked_action']:
            masked = np.zeros((infos.shape[0], infos.shape[1], self.envs.envs[0].env.config['total_action_dim']))
        ...
        self.buffer.insert(share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs, values, rewards,
                           masks, available_actions=masked)
