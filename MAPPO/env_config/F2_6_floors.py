...
'action_dim': 25, # 和上边对应
'elevator_action_dim': 6, # 电梯处多的动作，上楼和下楼
'super_action_dim': 1, # 有一些连不到的点，加到最后，使得地图联通
'total_action_dim': 32, # 25 + 6 + 1
...
'agent_num': 6, #TODO:对于大于2个agent的情况 还需要和代码对接 
'floor_num': 6,
'floor_list': [('site2', 'F2'), ('site2', 'F3'), ('site2', 'F4'),('site2', 'F5'), ('site2', 'F6'), ('site2', 'F7')],
'top_floor':('site2', 'F7'),
'bottom_floor':('site2', 'F2'),
'horizon': 250, # 记得要在MAPPO里的算法里同样改了 现在是250
'maximum_collect': 175, # 无人车最大一步收集能力
'render_collect' :175, # 实测时候的收集能力
...
'malicious_exist': False, # 是否用环境干扰
'use_malicious_evaluate': False, # 是否rend的时候用干扰转移
'use_masked_action': True, # 是否将撞墙的信息masked掉
'if_read_states': False, # 是否读入预存的states信息，如果要更新states信息记得False掉，重新存一遍
...
'uav_init_pos_2': (('site2', 'F6'), (580, 590)),

#奖励参数
'rewarding_methods':'default', # default 为基本设置

#算法参数
'use_dual_descent': False, # 是否使用对偶梯度
# TODO: 使用对偶梯度的时候貌似会导致
...
# VIME 部分参数
'use_vime': True,
'median_list_length': 40, # KL 归一化里 median 考虑的长度
'vime_reward_ratio': 20 , # KL 奖励权重，和参数量有关

# Elevator prediction 部分参数
'use_elevator': True,


#数据量和算法参数交叉部分
'use_changing_data': True, # 是否使数据按时间变化
'data_changes_num': 2, # 代表全局poi数据量刷新几次
'use_same_data': True, # 每次数据刷新是否用一样的数据，False的话需要去重新生成多个GP来预测, default:True
'data_last_time': 125, # 'horizon'/ 'data_changes_num',表示数据一次的持续时间
'dual_clip_value': 125, # 这个值应该设计为和data一次的持续时间时长一样， 'horizon'/ 'data_changes_num'
