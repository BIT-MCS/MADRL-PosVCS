...
class UAV(pygame.sprite.Sprite):
    ...
    def __init__(self, trajectory, screen, index=0):
        ...
        self.image = pygame.transform.smoothscale(pygame.image.load("./uav.png"), [10, 10])
    ...
    def update(self):
        ...
        for j in range(self.step - 1):
            a = (self.trajectory[j][0] * SCALE, self.trajectory[j][1] * SCALE)
            b = (self.trajectory[j + 1][0] * SCALE, self.trajectory[j + 1][1] * SCALE)
...
class PoIs(pygame.sprite.Sprite):
    ...
    def __init__(self, info, screen,full_info):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.transform.smoothscale( pygame.image.load("./poi_render.png"), [1, 1])
        self.rect = self.image.get_rect()
        self.screen = screen
        self.info = info
        self.step = 0
...
class Task(pygame.sprite.Sprite):
    ...
    def __init__(self, info, screen, task_info=None):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.transform.smoothscale(pygame.image.load("./F4_POI_on_grid_test.png"), [1, 1])
        self.rect = self.image.get_rect()
        self.screen = screen
        self.info = info
        self.step = 0
...
class RenderEnv(object):
    ...
    def __init__(self, info):
        ...
        self.bg = pygame.image.load('./concatenate_for_render.png')
        ...
        self.windows = pygame.display.set_mode((WIDTH * FLOOR_NUM * SCALE, HEIGHT * SCALE))
        ...
        self.NUM_STEP = len(info['reward_history'][0]) - 1
...
if __name__ == '__main__':
    from TSP.tsp_baseline import tsp_return
    info = tsp_return()
    R = RenderEnv(info)
    R.start()
